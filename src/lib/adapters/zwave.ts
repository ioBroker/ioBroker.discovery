...
export function detect(
    ...
    //      enum.functions: {}
    // }
    const baudRates = [115200]; // detect aeon labs
    tools.testSerialPort(
        ...
        function onOpen(port: any, callback: (error?: unknown) => void): void {
            try {
                port.write(Buffer.from([0x01, 0x03, 0x00, 0x15, 0xe9]));
                port.drain();
            } catch (e) {
        ...
        function onAnswer(
            ...
            if (data.length > 10 && data.length < 100) {
                while (data[j] !== 1 && j < data.length && j < 100) {
                    j++;
                }
                if (data[j + 1] === 0x10 && data[j + 2] === 0x01 && data[j + 3] === 0x15) {
                    j += 4;
                    while (data[j] && j < data.length && j < 100) {
                        version += String.fromCharCode(data[j++]);
                    }
                    if (version.includes('Z-Wave')) {
        ...
        function (err, found, name, baudRate, someInfo): void {
            if (found) {
                let instance = tools.findInstance(options, 'zwave', obj => obj.native.comName === name);
                if (!instance) {
                    instance = {
                        ...
                        comment: {
                            add: [
                                someInfo
                                    ? `${someInfo} ${tools.translate(options.language, 'on %s', comName)}`
                                    : `ZWave USB - ${comName}`,
                            ],
                        },
                    };
