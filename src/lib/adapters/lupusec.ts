...
function addDevice(ip: string, device: DiscoveryDevice, options: DetectOptions, callback: DetectCallback): void {
    let instance = tools.findInstance(
        options,
        'lupusec',
        obj => obj.native.alarm_host === ip || obj.native.alarm_host === device._name,
    );
...
export function detect(ip: string, device: DiscoveryDevice, options: DetectOptions, callback: DetectCallback): void {
    tools.httpGet(`http://${ip}/action/logout`, (err, data): void => {
        if (err || !data) {
            callback?.(null, false, ip);
        } else if (data) {
            let testData;
