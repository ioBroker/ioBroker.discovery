...
// exported value has to be larger than the internal one. With both at 700 ms the watchdog
// always wins the race and an answer arriving close to the limit would be thrown away.
const GOODWE_DETECT_TIMEOUT = GOODWE_TIMEOUT + 300;
...
function checksum16(data: ProtocolData, start: number, length: number): number {
    let checksum = 0;
...
export function buildIdInfoRequest(): Buffer<ArrayBuffer> {
...
export function isIdInfoResponse(data: ProtocolData): boolean {
...
function readAscii(data: ProtocolData, start: number, length: number): any {
    return data
...
function buildTitle(ip: string, response: ProtocolData): string {
...
function addInstance(ip: string, response: ProtocolData, options: DetectOptions): boolean {
...
export function detect(ip: string, device: DiscoveryDevice, options: DetectOptions, callback: DetectCallback): void {
    ...
    function finish(found: boolean): void {
        ...
        try {
            socket.close((): void => callback(null, found, ip));
        } catch (e) {
    ...
    socket.on('message', (message, remote): void => {
        // the socket is not bound to a peer, so answers of other hosts can arrive here as well
        if (remote.address !== ip) {
            options.log.debug(`GoodWe ignored a UDP answer of ${remote.address} while probing ${ip}`);
            return;
        }
...
export const type = ['ip']; // the inverter answers on UDP only, so every reachable IP is probed
export const timeout = GOODWE_DETECT_TIMEOUT;
