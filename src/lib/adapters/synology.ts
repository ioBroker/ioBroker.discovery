import * as tools from '../tools';
import type { DetectCallback, DetectOptions, DiscoveryDevice, DiscoveryInstance, ProtocolData } from '../types';

function getXml(url: string, cb: (...args: any[]) => void): void {
    tools.httpGet(url, (error, data): void => {
        if (!error && data) {
            const nameData = data.match(/<modelDescription>(.*?)<\/modelDescription>/);
            const modelData = data.match(/<modelNumber>(.*?)<\/modelNumber>/);
            const portData = data.match(/<URLBase.*>(.*?(\d+))<\/URLBase>/);
            const param = {
                name: nameData && nameData[1] ? nameData[1] : '',
                model: modelData && modelData[1] ? modelData[1] : '',
                port: portData && portData[2] ? portData[2] : '',
            };

            cb?.(param);
        } else {
            cb?.(null);
        }
    });
}

function addDevice(ip: string, upnp: ProtocolData, options: DetectOptions, callback: DetectCallback): boolean {
    const instance = tools.findInstance(options, 'synology', obj => obj.native.host === ip);

    if (instance) {
        return false;
    }
    getXml(upnp.LOCATION, param => {
        let instance = tools.findInstance(options, 'synology', obj => obj.native.host === ip);
        if (!instance && param) {
            const id = tools.getNextInstanceID('synology', options);
            instance = {
                _id: id,
                common: {
                    name: 'synology',
                    enabled: false,
                    title: (obj: DiscoveryInstance): any => obj.common.title,
                },
                native: {
                    host: ip,
                    port: param.port,
                    login: '',
                    password: '',
                    https: false,
                    version: '',
                    polling: 5000,
                },
                comment: {
                    add: [param.name, param.model, ip],
                },
            };

            options.newInstances.push(instance);

            callback?.(null, true, ip);
        } else {
            callback?.(null, false, ip);
        }
    });
    return true;
}

export function detect(ip: string, device: DiscoveryDevice, options: DetectOptions, callback: DetectCallback): void {
    for (const upnp of device._upnp) {
        if (upnp.ST === 'upnp:rootdevice' && upnp.SERVER.includes('Synology')) {
            options.log.debug(`Synology UPnP Data: ${JSON.stringify(upnp)}`);
            if (addDevice(ip, upnp, options, callback)) {
                return;
            }
        }
    }

    callback(null, false, ip);
}

export const type = ['upnp'];
export const timeout = 5000;
