...
function addInstance(device: DiscoveryDevice, options: DetectOptions, cb: (...args: any[]) => void): void {
    ...
    if (!instance) {
        const comment = `Cameras/NVT - ${device.count} (${tools.translate(options.language, 'comments')})`;
...
type ProbeCallback = (error: unknown, cams: ProtocolData[] | null) => void;

function discoveryProbe(options: DetectOptions, callback: ProbeCallback | null): void {
    const cams: ProtocolData[] = [];
    const messageID = `urn:uuid:${guid()}`;
    const request = Buffer.from(
        ...
        socket = require('node:dgram').createSocket('udp4');
    ...
    const listener = function (msg: ProtocolData, rinfo: RemoteInfo): void {
...
export function detect(
    ...
    discoveryProbe(options, (err, cams): void => {
        if (err) {
            cb?.(err, false, ip);
        } else {
            let ports = '';
            ...
            let end_range;
            let count = 0;
            cams!.forEach((device: ProtocolData): void => {
                if (!device.port && !device.ip) {
                    return;
                }
                count++;
                if (device.port && ports.indexOf(device.port) < 0) {
                    if (ports.length > 1) {
                        ports = `${ports}, ${device.port}`;
                    } else {
                        ports = device.port;
                    }
                }
                if (device.ip) {
                    if (ip_list.indexOf(device.ip) === -1) {
            ...
            if (count == 0) {
                ...
                addInstance(device, options, (state: boolean): void => {
                    cb?.(null, state, ip);
                });
            }
        }
