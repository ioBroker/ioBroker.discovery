...
function crc16(data: ProtocolData): number {
    let crc = 0xffff;
...
function buildInfoRequest(): Buffer<ArrayBuffer> {
...
function addInstance(ip: string, dtuSn: string, options: DetectOptions): boolean {
...
export function detect(ip: string, device: DiscoveryDevice, options: DetectOptions, callback: DetectCallback): void {
    ...
    socket.on('connect', (): boolean => socket.write(buildInfoRequest()));
