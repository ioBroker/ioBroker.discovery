...
function cleanup(ip: string, callback: DetectCallback | null, callbackResult: boolean): void {
    if (client) {
        client.close((): void | undefined => callback?.(null, callbackResult, ip));
...
function checkMessageType(message: Buffer): boolean {
...
export const type = ['once']; // SMA Energy Meters send out multicast messages on regular intervals
export const timeout = EM_TIMEOUT + 100; // Make sure that internal timeout strikes first
