...
let tools: typeof toolsModule | undefined;
try {
...
let knxAdr: string | undefined;
let deviceName: string | undefined;

function convertToKnxAddr(adr: number): string {
...
function addDevice(
    ...
    if (options) {
        const instance = tools!.findInstance(
            options,
            adapterName,
            (obj: DiscoveryInstance): boolean => obj.native.gwip === ip,
        );
...
export function detect(
    ...
    addresses.forEach(ip => {
        udpScan(mcast, mport, ip, undefined, openknxTimeout, false, (err, msg): void => {
            if (!msg) {
                ...
                deviceName = String.fromCharCode(...dib_dev_friendly_name);
...
type UdpScanCallback = (error: unknown, message: Buffer | null, remote?: udp.RemoteInfo) => void;
...
function udpScan(
    ...
    udpSocket.on('message', (message, remote): void => {
        console.log(`UDP Discovery response:${remote.address}:${remote.port} - ${String(message)}`);
        if (onlyOneResult) {
            clearTimeout(probeTimeout);
            try {
                udpSocket.close();
