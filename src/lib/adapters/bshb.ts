...
export function detect(
    ...
    requestOptions.checkServerIdentity = (host: string): false | undefined => {
        ...
        // Known bug: the discovery logger has `warn`, not `warning` - this throws on the
        // verification-failed path. Kept as is, to be fixed separately.
        (options.log as any).warning(
            `Lookup Error from BSHB: Hostname verification failed. server=${host} expected=${this.host}`,
        );
        return false;
    };
