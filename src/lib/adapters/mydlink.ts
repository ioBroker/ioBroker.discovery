...
function addDevice(ip: string, device: DiscoveryDevice, options: DetectOptions, callback: DetectCallback | null): void {
    ...
    if (!instance) {
        ...
        if (!instance.comment) {
            //existing instance but was not yet touched by discovery.
        ...
        if (instance.comment.add) {
            //if we added the instance before, it will have "add" and we can add the device there, too.
            instance.comment.add.push(name);
        }
        if (instance.comment.extended) {
            //if it was existing instance, we need to extend it. Right???
...
function extractStringsFromBuffer(buffer: Buffer): string[] {
    ...
    for (const key of Object.keys(buffer)) {
        //console.log('key:', key);
        if (nextStop < 0) {
            nextStop = Number.parseInt(key, 10) + buffer[Number(key)];
            //console.log('Next str length: ', buffer[key], 'nextstop:', nextStop);
        } else {
            tmpStr += String.fromCharCode(buffer[Number(key)]);
...
export const timeout = 30000; //doesn't do anything, mdns timeout is hardcoded to 15000 in mdns.js (very bottom).
