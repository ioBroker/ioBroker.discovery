...
function discoverUpnp(self: MethodInstance): void {
    ...
    multicast_client.on('advertise-alive', (headers: ProtocolData, rinfo: RemoteInfo): any =>
        self.parseMessage(headers, 0, rinfo),
    );
    client.on('response', (headers: ProtocolData, statusCode: unknown, rinfo: RemoteInfo): any =>
        self.parseMessage(headers, statusCode, rinfo),
    );
    multicast_client.on('response', (headers: ProtocolData, statusCode: unknown, rinfo: RemoteInfo): any =>
        self.parseMessage(headers, statusCode, rinfo),
    );
...
export const browse = discoverUpnp;
export const type = 'upnp';
export const source = 'upnp';
