 ...
 *
 * @alias getOwnAddress
 * @param {string} ip ip address of a device that we want to connect to
 * @returns {string} own ip address of the interface which we can use to communicate with a desired device
 */
...
/**
 * Gets an array of all valid (not internal) IPv4 addresses and names of this host
 *
 * @returns {Record<name: string, ip: string>[]}
 */
...
function getBroadcastAddresses() {
    ...
    const broadcastAddresses = Object.keys(net)
        ...
        // extract address and subnet as the array of numbers
        .map(k => ({
 ...
 *
 * This function tries to open a given TCP port, and if onConnect and onReceive are defined, the user can send some data in onConnect and to check it in onReceive.
 *
 * @alias testPort
 ...
 *
 * First, it sends UDP Multicast to detect devices with defined ST to port 1900.
 *
...
function ssdpScan(ip, text, readXml, timeout, probePort, callback) {
    ...
    dgram = dgram || require('node:dgram');
...
function udpScan(probeAddress, probePort, listenAddress, listenPort, probeData, timeout, onlyOneResult, callback) {
    ...
    udpSocket.on('message', (message, remote) => {
        ...
        callback?.(null, message.toString(), remote);
        if (onlyOneResult) {
            callback = null;
        }
    });
    ...
    if (options && options.existingInstances) {
        for (let k = 0; k < options.existingInstances.length; k++) {
            if (options.existingInstances[k].common && options.existingInstances[k].common.name === name) {
                instances.push(
                    parseInt(options.existingInstances[k]._id.substring(`system.adapter.${name}.`.length), 10),
                );
            }
        }
    }
...
function findInstance(options, name, compare) {
    for (let i = 0; i < options.existingInstances.length; i++) {
        if (
            options.existingInstances[i].common &&
            options.existingInstances[i].common.name === name &&
    ...
    for (let i = 0; i < options.newInstances.length; i++) {
        if (
            options.newInstances[i].common &&
            options.newInstances[i].common.name === name &&
            (!compare || compare(options.newInstances[i]))
        ) {
            const instance = options.newInstances[i];
            return instance;
        }
    }
    return null;
...
function httpGet(link, timeout, callback) {
    ...
    if (!link) {
        callback?.('error: no link provided', null, link);
        callback = null;
    }
    ...
    try {
        const req = HTTP.get(link, res => {
            ...
            res.on('end', () => callback?.(null, rawData ? rawData.toString() : null, link));
        }).on('error', e => callback?.(e.message, null, link));

        req.setTimeout(timeout, () => {
            req.abort();
            callback?.('timeout', null, link);
            callback = null;
        });
    } catch (err) {
        callback?.(`error: ${err}`, null, link);
        callback = null;
    }
}
    ...
    if (typeof baudRates === 'object') {
        if (!baudRates || !baudRates.length) {
            ...
            testSerialPort(name, options, baudRate, onOpen, onReceived, (err, found, name, baudrate, someInfo) => {
                if (found) {
                    serialDebug && options.log.error(`------------------- <<< <<< <<< <1< testSerialPort "${name}`);
        ...
        serialDebug &&
            options.log.error(`------------------- >>> >>> >>> >>> >>> testSerialPort "${name} - ${baudRates}`);
        let timeout;
        let closing = false;
        let closePort;
        try {
            ...
            closePort = (code, found) => {
                ...
                try {
                    if (port && port.isOpen) {
                        serialDebug && options.log.debug(`${new Date()} close port ${name} ${baudRates}`);
                        closing = true;
                        port.close(() => {
                            if (callback) {
                                serialDebug &&
            ...
            port.on('error', err => {
                if (err) {
                    serialDebug && options.log.error(`Error on port ${name}: ${err}`);
                    closePort(6);
                }
            });
            ...
            port.open(err => {
                if (err) {
                    serialDebug && options.log.warn(`Cannot open port, ${name}: ${err}`);
                    closePort(7);
                }
            });
        } catch (err) {
...
function getLocationDesc(device, locationUrl, callback) {
    ...
    if (!locationUrl) {
        return callback?.('no location url');
    }
