...
function udpScan(probeAddress, probePort, listenAddress, listenPort, probeData, timeout, callback) {
    ...
    const probeTimeout = setTimeout(function () {
        udpSocket.close();
...
/**
 * Safe wrapper around Buffer.from that works in oder NodeJS versions
 * @param {any[] | string} source The data to construct the buffer from
...
/**
 * Safe wrapper around Buffer.alloc that works in oder NodeJS versions
 * @param {number} length The length of the Buffer to create
 */
function Buffer_alloc(length) {
    if (Buffer.alloc) {
