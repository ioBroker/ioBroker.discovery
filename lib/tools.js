 ...
 * @alias getOwnAddress
 * @memberof tools
 * @param {string} ip ip address of a device which we want to connect to
 * @return {string} own ip address of the interface which we can use to communicate with a desired device
 */
            ...
            closePort = (code, found) => {
                ...
                try {
                    if (port && port.isOpen) {
                        ...
                        port.close(() => {
                            if (callback) {
                                serialDebug && options.log.error(`------------------- <<< <<< <<< <<< <${code}< testSerialPort "${name} - ${baudRates}`);
                ...
                port = null;
                if (callback) {
                    serialDebug && options.log.error(`------------------- <<< <<< <<< <<< 1${code}< testSerialPort "${name} - ${baudRates}`);
...
function getLocationDesc(device, locationUrl, callback) {
    ...
    if (!locationUrl) {
        return callback && callback('no location url');
    }
