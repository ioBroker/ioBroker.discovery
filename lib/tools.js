...
function testSerialPort(name, options, baudRates, onOpen, onReceived, callback) {
    if (typeof baudRates === 'object') {
        if (!baudRates || !baudRates.length) {
            options.log.warn('Stop scan port ' + name);
            callback('not found', false, name);
        } else {
            var baudRate = baudRates.shift();
            options.log.warn('Call port ' + name + ' ' + baudRate);
        ...
        var isOpened;
        try {
            options.log.warn('Open port ' + name);
            ...
            port.on('open', function() {
                isOpened = true;
                options.log.warn('port ' + name + ' opened: ' + options.baudRate);
                        ...
                        if (err) {
                            ...
                            setTimeout(function () {
                                try {
                                    ...
                                    options.log.warn('Cannot close port ' + name + ': ' + e);
                                }
                                port = null;
                    ...
                    try {
                        onReceived(port, data, function (err, found, isStop, someInfo) {
                            if (err || isStop || found) {
                                ...
                                port = null;
                                if (callback) callback(err, found, name, baudRate, someInfo);
                                callback = null;
                            }
                        });
                    }catch (e) {
                        options.log.warn('Cannot close port ' + name + ': ' + e);
                        if (callback) callback(e, false, name, baudRate);
                        callback = null;
                    }
            ...
            port.on('error', function (err) {
                options.log.error('Error on port ' + name + ': ' + err);
                try {
                    if (port && port.isOpen()) {
                        port.close();
                    }
            ...
            port.open(function (err) {
                options.log.warn('Cannot open port, ' + name + ': ' + err);
                if (err && callback) {
                    callback(err);
                    callback = null;
                }
            ...
            options.log.error('Cannot open_ port ' + name + ': ' + err);
            if (callback) {
                callback(err);
                callback = null;
            }
