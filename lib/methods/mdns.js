...
function old_browse (options, log, updateProgress, addDevice) {
    ...
    var mdns = new Mdns ({
        ...
        name: [ '_services._dns-sd._udp.local', '_raop._tcp.local', '_sleep-proxy.-udp', '_homekit._tcp', '_amzn-wplay._tcp.local', '_http._tcp.local', '_mieleathome._tcp', '_services._dns-sd._udp.local', '_touch-able._udp', 'soundtouch' ],
        find: '*',
        broadcast: false
    });
    mdns.noQuestions = true;
    ...
    mdns.on('entry', function(entry) {
        ...
        log.debug('Discovered mDNS device: ' + JSON.stringify(device));
