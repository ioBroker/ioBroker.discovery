...
function pingBlocks(options, log, blocks, totalLength, progressCallback, basePercent, totalRanges, callback, _result) {
    ...
    _result = _result || [];
    if (progressCallback) {
        progressCallback('ping', basePercent + Math.round(((totalLength - blocks.length) * 100 / totalLength) / totalRanges));
    }

    var block = blocks.shift();
    pingBlock(options, log, block, function (err, result) {
        _result = _result.concat(result);
        setTimeout(pingBlocks, 0, options, log, blocks, totalLength, progressCallback, basePercent, totalRanges, callback, _result);
    });
}
...
function _pingAll(options, log, progressCallback, callback, ranges, totalRanges, _result) {
    dns  = dns  || require('dns');
    ...
    if (!ranges) {
        ...
        return _pingAll(options, log, progressCallback, callback, ranges, ranges.length, _result);
    }
    ...
    if (!ranges.length) {
        ...
        pingRange(options, log, range.ip, range.mask, progressCallback, 100 - Math.round(((ranges.length + 1) / totalRanges) * 100), totalRanges, function (err, result) {
            if (result) _result = _result.concat(result);
            setTimeout(_pingAll, 0, options, log, progressCallback, callback, ranges, totalRanges, _result);
        });
    }
}
