...
function discover(options, log, progressCallback, callback) {
    
    var percent = 0;
    function updateProgress(_percent) {
        percent = _percent;
        progressCallback(methodName, _percent, g_resultCount = 0);
    }
    ...
    exports.scanForAllDevices(
        ...
        function(result) {
            ...
            log.info('Done discovering ' + methodName + ' devices. ' + result.length + ' devices found');
            updateProgress(100);
            typeof callback === 'function' && callback(null, result, methodName, 'ip');
            callback = null;
        }
    )
}
