    ...
    if (!found) {
        if (data && data.LOCATION && data.LOCATION.match(/^http:\/\/.+\.xml$/)) {
            tools.httpGet(data.LOCATION, function (err, _data) {
                if (_data) {
                    const mRoom = _data.match(/<roomName>(.+)<\/roomName>/);
                    const mName = _data.match(/<displayName>(.+)<\/displayName>/);
                    let name;
                    let room;
                    if (mRoom && mRoom[1]) {
                        room = mRoom[1];
...
function detect(ip, device, options, callback) {
    ...
    if (device._type === 'ip') {
        ...
        tools.ssdpScan(ip, text, (err, result) => {
            if (result && result.SERVER && result.SERVER.indexOf('Sonos') !== -1) {
                ...
                //    }
                //
                addSonos(ip, result, options, isAdded => {
                    if (callback) {
