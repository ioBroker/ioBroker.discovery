    ...
    instance = {
      ...
      comment: {
        ...
        inputs: [{
            ...
            title: 'username' // tools.translate(options.language, 'alarm_user') // see translation in words.js
          },
          {
            name: 'native.alarm_password',
            def: '',
            type: 'password', // text, checkbox, number, select, password. Select requires
            title: 'password' // tools.translate(options.language, 'alarm_password') // see translation in words.js
          }
        ]
      }
    };
...
function detect(ip, device, options, callback) {
  ...
  tools.httpGet('http://' + ip + '/action/logout', function(err, data) {
    if (err) {
      ...
      if (testData && testData.hasOwnProperty('result') && testData.hasOwnProperty('message')) {
