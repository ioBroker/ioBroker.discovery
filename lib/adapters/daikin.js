...
function detect(ip, device, options, callback) {
    ...
    tools.udpScan(ip, 30050, '0.0.0.0', 30000, 'DAIKIN_UDP/common/basic_info', 1000, (err, result) => {
        if (!result) {
            if (callback) {
        ...
        if (responseData && responseData.ret && responseData.ret === 'OK') {
            addDaikin(ip, device, responseData, options, isAdded => {
                if (callback) {
...
exports.type    = ['ip'];// make type=serial for USB sticks // TODO make to once
