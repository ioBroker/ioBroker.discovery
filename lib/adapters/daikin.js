...
function detect(ip, device, options, callback) {
    tools.udpScan(ip, 30050, '0.0.0.0', 30000, 'DAIKIN_UDP/common/basic_info', 3000, (err, result, remote) => {
        if (!result) {
            if (callback) {
        ...
        if (responseData && responseData.ret && responseData.ret === 'OK') {
            const found = addDaikin(remote.address, device, responseData, options);
            if (callback) {
