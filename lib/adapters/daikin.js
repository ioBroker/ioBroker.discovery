...
function addDaikin(ip, data, options, callback) {
    var foundNew = false;
    var instance = tools.findInstance(options, 'daikin', function (obj) {
        //TODO handling 127.0.0.1 vs other IP
        var matchFound = (obj.native.daikinIp === ip);
        options.log.debug('Check existing Dikin instances for Device ' + ip + ':' + matchFound);
        return matchFound;
    });
...
function detect(ip, device, options, callback) {
    ...
    tools.udpScan(ip, 30050, '0.0.0.0', 30000, 'DAIKIN_UDP/common/basic_info', 1000, function (err, result) {
        ...
        if (responseData && responseData.ret && responseData.ret === 'OK') {
            addDaikin(ip, responseData, options, function (isAdded) {
                if (callback) {
