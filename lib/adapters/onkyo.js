...
function detect(ip, device, options, callback){
    ...
    let onkyoPacket =  new Buffer.from([73, 83, 67, 80, 0, 0, 0, 16, 0, 0, 0, 11, 1, 0, 0, 0, 33, 120, 69, 67, 78, 81, 83, 84, 78, 13, 10]);
    let pioneerPacket = new Buffer.from([73, 83, 67, 80, 0, 0, 0, 16, 0, 0, 0, 11, 1, 0, 0, 0, 33, 112, 69, 67, 78, 81, 84, 83, 78, 13, 10]);

    tools.udpScan(ip, 60128, '0.0.0.0', 1235, onkyoPacket, 5000, (err, data, remote) => {
        if (!err && data && remote){
            let type = 'onkyo';
            let message = data.toString().slice(18, data.length - 6), command = message.slice(0, 3), _data;
            if (command === 'ECN'){
