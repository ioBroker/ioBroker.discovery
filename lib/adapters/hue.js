...
function detect(ip, device, options, callback) {
    ...
    if (device._upnp && (device._upnp['HUE-BRIDGEID'] || device._upnp['hue-bridgeid']) && device._upnp.LOCATION && device._upnp.LOCATION.indexOf(ip) !== -1) {
        // {
        ...
        // }
        addHue(ip, device, options, isAdded => callback && callback(null, isAdded, ip));
    } else {
