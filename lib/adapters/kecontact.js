...
function addInstance(ip, device, options, native) {
    ...
    if (instance) {
        options.log.info('Keba KeContact adapter already present for IP ' + options.ip);
    } else {
        instance = {
...
function detect(ip, device, options, callback) {
    ...
    rxSocket.on('listening', function () {
        ...
        txSocket.send(DETECT_MESSAGE, 0, DETECT_MESSAGE.length, DEFAULT_UDP_PORT, '192.168.97.47', (err, bytes) => err && options.log.warn('Error from KeContact: ' + err));
    });
