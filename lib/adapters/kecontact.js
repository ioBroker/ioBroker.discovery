...
function addInstance(ip, device, options) {
    ...
    if (instance) {
        ...
        instance = {
            ...
            comment: {
                add: ["control you Keba KeContact P30 charging station"]
            }
        };
...
function detect(ip, device, options, callback) {
    options.log.info('Detecting Keba KeContact wallbox on ' + ip + '...');
    ...
    rxSocket.on('listening', function () {
        ...
        rxSocket.send(DETECT_MESSAGE, 0, DETECT_MESSAGE.length, DEFAULT_UDP_PORT, ip, (err, bytes) => err && options.log.warn('Error from KeContact: ' + err));
    });
    rxSocket.on('message', (message, remote) => {
        options.log.debug('UDP datagram from ' + remote.address + ':' + remote.port + ': "' + message + '"');
        if (message == DETECT_MESSAGE) {
            return;    // broadcast message received by myself ...
        }
