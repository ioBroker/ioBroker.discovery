...
function detect(ip, device, options, callback) {
    ...
    options.log.debug(device);
    if (device._upnp && device._upnp['USN'].indexOf('BOSE') !== -1 && device._upnp.LOCATION && device._upnp.LOCATION.indexOf(ip) !== -1) {
        options.log.debug('Found new Bose Soundtouch Instance' + ip);
