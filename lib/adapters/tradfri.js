...
function detect(ip, device, options, callback) {
	...
	if (srv.name !== fullHostname || !hostnameIsTradfri(fullHostname)) return fail();

	// The SRV must have a port of 5684
	if (srv.data.port != 5684) return fail(); // != because we don't know if string or number

	// And the A(AAA) record must contain "TRADFRI"
	const ARecord = device._mdns.A || device._mdns.AAAA;
	if (!startsWithTRADFRI(ARecord.name)) return fail();
