...
function listen(ip, options, cb) {
    tools.udpScan(ip, 4321, '0.0.0.0', 9898, '{"cmd": "whois"}', 500, (err, msg) => {
        options.log.debug(msg);
        msg = JSON.parse(msg);

        if (msg && msg.model && msg.model === 'gateway') {
            options.log.debug('mihome1: ' + ip);
            options.log.debug('mihome1: ' + JSON.stringify(msg));
...
exports.type = ['ip']; // TODO check if udp
exports.timeout = 500;