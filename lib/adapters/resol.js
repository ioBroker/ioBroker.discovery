...
function detect(ip, device, options, callback) {
    ...
    debug && options.log.debug(`Device Information URL ${devInfoURL}`);

    tools.httpGet(devInfoURL, 2000, (err, data) => {
        //debug && options.log.debug(`Device Information of ${ip} ` + data);
        ...
        if (!err && data && devInfo) {
            ...
            if (!instance) {
                ...
                // debug && options.log.debug(`Device ` + name + ' ' + JSON.stringify(device));
