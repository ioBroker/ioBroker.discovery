...
function detect(ip, device, options, callback) {
    ...
    tools.testPort(ip, 3493, {
        ...
        if (found) {
            var foundNew = false;
            for (var found_ups in foundNutServer[ip]) {
                var instance = tools.findInstance(options, 'nut', function (obj) {
                    //TODO handling 127.0.0.1 vs andere IP
                    var matchFound = (obj.native.host_ip === ip && obj.native.host_port === 3493 && obj.native.ups_name == found_ups);
                    options.log.debug('Check existing NUT instances for UPS ' + found_ups + '@' + ip + ':' + matchFound);
                    return matchFound;
                });
                if (!instance) {
                    foundNew = true;
                    instance = {
                        ...
                        common: {
                            name: 'nut',
                            title: 'Network UPS Adapter (' + found_ups + '@' + ip + ' ' + foundNutServer[ip][found_ups] + ')'
                        },
                        native: {
                            host_ip: ip,
                            host_port: 3493,
                        ...
                        comment: {
                            add: true,
                            text: found_ups + '@' + ip + ' (' + foundNutServer[ip][found_ups] + ')'
                        }
                    };
