...
function detect(ip, device, options, callback) {
    ...
    server.on('listening', () => {
        ...
        server.send(new Buffer(request), 0, request.length, 3671, ip, (err, bytes) => {
            if (err) throw err;
        });
    });
    ...
    server.on('message', (message, remote) => {
        ...
        if (message && message[2] === 2 && message[3] === 6) { // if CONNECT_RESPONSE:  0x0206,
            ...
            server.send(new Buffer(disconnect), 0, disconnect.length, 3671, ip, err => {
                server.close(() => {
...
exports.type    = ['ip'];// make type=serial for USB sticks // TODO check if maybe upnp?
exports.timeout = 600;
