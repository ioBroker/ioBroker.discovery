...
function addDeconz(ip, device, options, callback) {
    ...
    var instance = tools.findInstance(options, 'deconz', function (obj) {
        if (obj && obj.native && (obj.native.bridge === ip || obj.native.webServer === device._name)) {
            return true;
        }
    });
...
function detect(ip, device, options, callback) {
    ...
    if (device._upnp && device._upnp['GWID.PHOSCON.DE'] && device._upnp.LOCATION && device._upnp.LOCATION.indexOf(ip) !== -1) {
        // {
        ...
        // }
        addDeconz(ip, device, options, function (isAdded) {
