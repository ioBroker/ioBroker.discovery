...
function startAdapter(options) {
  ...
  Object.assign(options, { name: "discovery" });
...
function forEachValidAdapter(device, dependencies, callback) {
  ...
  for (const a in adapters) {
    if (
      Object.prototype.hasOwnProperty.call(adapters, a) &&
      isValidAdapter(a, type, dependencies)
    ) {
    ...
    (function (adpr) {
      ...
      adapters[adpr].detect(
        ...
        (err, isFound /* , addr */) => {
          ...
          if (isFound) {
            adapter.log.debug(
              `Test ${device._type} ${device._addr} ${adpr} DETECTED!`,
            );
          }
          if (timeout) {
...
function analyseDevice(device, options, callback) {
  ...
  if (device._type === "serial") {
    ...
    analyseDeviceSerial(device, options, list, () =>
      analyseDeviceDependencies(device, options, callback),
    );
  } else {
    const callbacks = {};
    ...
    forEachValidAdapter(device, false, (_adapter, a) => {
      (function (adpr) {
        ...
        try {
          ...
          adapter.log.error(
            `Cannot detect "${adpr}" on "${device._addr}": ${e}`,
          );
          if (count !== false && !--count) {
            analyseDeviceDependencies(device, options, callback);
            count = false;
...
function analyseDevices(devices, options, index, callback) {
  ...
  adapter.setState(
    "servicesProgress",
    Math.round((index * 100) / devices.length),
    true,
  );
  ...
  adapter.getForeignObject("system.repositories", (err, repo) => {
    adapter.getForeignObject("system.config", (err, systemConfig) => {
      ...
      if (
        ...
        repo.native.repositories[systemConfig.common.activeRepo] &&
            ...
            analyseDevices(devices, options, 0, async (/* err */) => {
                ...
                if (instance.encryptedNativeLegacy) {
                  ...
                  list.forEach((attr) => {
                    if (instance.native[attr]) {
                      instance.native[attr] = adapter.encrypt(
                        secret,
                        instance.native[attr],
                      );
                    }
                  });
                }
              ...
              for (let j = oldInstances.length - 1; j >= 0; j--) {
                if (oldInstances[j].comment.ack) {
                  ...
                  oldInstances[j]._id = oldInstances[j]._id.replace(
                    /\.\d+$/,
                    "",
                  );
              ...
              isRunning = false;
              err &&
                adapter.log.error(`Cannot update system.discovery: ${err}`);
...
const Method = function (methodName, parent) {
  ...
  this.updateProgress = function (progress) {
    ...
    adapter.log.debug(
      `${self.source}: ${self.progress}%, devices - ${self.foundCount}`,
    );
  ...
  this.done = function (err) {
    ...
    self.progress = 100;
    adapter.log.info(
      `Done discovering ${self.source} devices. ${self.foundCount} packages received`,
    );
    parent.done(self);
  };
...
function browse(options, callback) {
  ...
  function Browse() {
    ...
    const methodsArray = Object.keys(methods).filter(
      (m) => methods[m].browse && (!options || options.includes(m)),
    );
    ...
    this.updateProgress = function () {
      ...
      timeoutProgress = setTimeout(() => {
        ...
        adapter.setState(
          "devicesProgress",
          Math.round(value / methodsArray.length),
          true,
        );
    ...
    this.done = function (method) {
      ...
      if (!self.count) {
        ...
        Object.keys(g_devices)
          .sort()
          .forEach((t) =>
            Object.keys(g_devices[t])
              .sort()
              .forEach((d) => devices.push(g_devices[t][d])),
          );
    ...
    this.getMissedNames = function (devices, callback) {
      ...
      (function doIt() {
        ...
        dns.reverse(dev._addr, (err, hostnames) => {
          dev._name =
            !err && hostnames && hostnames.length ? hostnames[0] : dev._sddr;
          dev._dns = {
            hostnames: hostnames,
          };
          doIt();
    ...
    this.addDevice = function (newDevice, source /* methodName */, type) {
      ...
      if (old && old._type === type) {
        device = old;
        adapter.log.debug(
          `extended Device: ${newDevice._addr} source=${source}`,
        );
        ...
        adapter.log.debug(
          `main.addDevice: ip=${newDevice._addr} source=${source}`,
        );
      ...
      (function _merge(dest, from) {
        Object.getOwnPropertyNames(from).forEach((name) => {
          ...
          if (typeof from[name] === "object") {
            ...
            const namex = `${name}x`;
            if (
              specialEntryNames.includes(name) &&
              dest[name] &&
              from[name] !== undefined &&
